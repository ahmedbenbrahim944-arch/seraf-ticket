...
export class PrintService {
  ...
  /**
   * ✅ MODIFICATION PRINCIPALE: Utiliser DMATRIX au lieu de QRCODE
   */
  printWithLineTSPL(tickets: any[], copies: number = 1): Promise<{ success: boolean; message: string }> {
    return new Promise((resolve, reject) => {
      try {
        ...
        console.log(`Génération TSPL DataMatrix pour ${tickets.length} tickets`);
  ...
  /**
   * ✅ MODIFICATION CRITIQUE: Remplacer QRCODE par DMATRIX
   */
  private generateSingleTicketCommandsWithPixelOffset(ticket: any, offsetXMm: number): string[] {
    ...
    // ✅ DATAMATRIX au lieu de QRCODE
    if (ticket.fullProductNumber) {
      const dmX = this.printPositionConfig.dataMatrixX + offsetXPx;
      const dmY = this.printPositionConfig.dataMatrixY;
      
      // ✅ Commande DMATRIX pour imprimantes TSC
      // Format: DMATRIX x,y,width,height,xm,r,c,s,d"data"
      // xm = module size (x8 = 8 dots)
      // r = rotation (0-3)
      // c = columns (0 = auto)
      // s = rows (0 = auto)
      commands.push(`DMATRIX ${dmX},${dmY},${this.printPositionConfig.dataMatrixSize},${this.printPositionConfig.dataMatrixSize},x8,0,0,0,d"${ticket.fullProductNumber}"`);
    }

    // Code produit complet
    if (ticket.fullProductNumber) {
    const textX = this.printPositionConfig.fullProductNumberX + offsetXPx;
    const textY = this.printPositionConfig.fullProductNumberY; // Maintenant à 60px
  ...
  /**
   * ✅ Version individuelle avec DMATRIX
   */
  private generateIndividualTicketsTSPL(tickets: any[], copies: number = 1): string {
    ...
    tickets.forEach((ticket, index) => {
      ...
      // ✅ DMATRIX au lieu de QRCODE
      if (ticket.fullProductNumber) {
        commands.push(`DMATRIX ${this.printPositionConfig.dataMatrixX},${this.printPositionConfig.dataMatrixY},${this.printPositionConfig.dataMatrixSize},${this.printPositionConfig.dataMatrixSize},x8,0,0,0,d"${ticket.fullProductNumber}"`);
      }
      ...
      if (ticket.fullProductNumber) {
        const displayText = ticket.fullProductNumber.length > 16 ? 
          ticket.fullProductNumber.substring(0, 16) : ticket.fullProductNumber;
        commands.push(`TEXT ${this.printPositionConfig.fullProductNumberX},${this.printPositionConfig.fullProductNumberY},"${this.printPositionConfig.fullProductNumberFontSize}",0,1,1,"${displayText}"`);
      }
  ...
  // ✅ Mise à jour de l'HTML pour l'aperçu - Remplacer qr-code par datamatrix
  private generatePrintableHTML(): string {
    ...
    return `
      ...
      <html>
        <head>
          <title>Étiquettes TSC avec DataMatrix - Format Ligne 65x10mm</title>
          <style>
            @page { 
              size: ${lineWidth}mm ${lineHeight}mm;
              margin: 0mm; 
  ...
  private generateSingleTicketHTML(ticket: any, ticketNumber: number): string {
    ...
    // ✅ Utiliser dataMatrix au lieu de qrCode
    const codeImage = ticket.codeImage || ticket.qrCode || '';
    
    return `
      <div class="ticket-label" >
        <div class="datamatrix-section">
          <img src="${codeImage}" alt="DataMatrix-$" class="datamatrix-code">
  ...
  debugPrintSettings(): void {
    console.group('Configuration d\'impression avec DataMatrix');
  ...
  // ✅ API Methods - Mise à jour pour utiliser dataMatrix
  ...
  // ✅ Mise à jour: dataMatrix au lieu de qrCode
  getTicketDataMatrix(id: number): Observable<any> {
    return this.http.get(`${this.apiUrl}/print/ticket/${id}/datamatrix`);
  }
