...
export class Xt6PrintService {
  ...
  private printPositionConfig: PrintPositionConfig = {
    ...
    productCodeY: 38,        // 4.75mm = 38px
    productCodeFontSize: 1,

    // Texte XT6
    xt6TextX: 42,            // 5.25mm = 42px
    xt6TextY: 18,            // 2.25mm = 18px
    xt6FontSize: 2,          // Taille moyenne

    // Matricule (Unique Code)
    matriculeX: 42,          // 5.25mm = 42px
    matriculeY: 28,          // 3.5mm = 28px
    matriculeFontSize: 1,

    // Date - Position optimisée pour 17x7mm
    dateX: 42,               // 5.25mm = 42px
    dateY: 48,               // 6mm = 48px
    dateFontSize: 1          // Petite taille
  };
  ...
  /**
   * Formate la date au format JJ/MM/AA
   */
  ...
  /**
   * Prépare les données de ticket avec la date formatée
   */
  prepareTicketsWithDate(tickets: any[]): any[] {
    return tickets.map(ticket => {
      const printDate = ticket.printDate ? new Date(ticket.printDate) : new Date();
      return {
        ...ticket,
        formattedDate: this.formatDateShort(printDate)
      };
    });
  }
  ...
  printSuccessiveTSPL(tickets: any[], copies: number = 1): Promise<{ success: boolean; message: string }> {
    return new Promise((resolve, reject) => {
      try {
        ...
        console.log(`Génération TSPL successive pour ${tickets.length} tickets XT6 (17x7mm)`);
        
        // Préparer les tickets avec la date formatée
        const ticketsWithDate = this.prepareTicketsWithDate(tickets);
        const tsplCommands = this.generateSuccessiveTSPL(ticketsWithDate, copies);
  ...
  private generateSuccessiveTSPL(tickets: any[], copies: number = 1): string {
    ...
    tickets.forEach((ticket, index) => {
      console.log(`Génération ticket XT6 ${index + 1}/${tickets.length}:`, ticket.progressiveNumber);
      ...
      // Texte XT6
      const xt6TextX = this.printPositionConfig.xt6TextX;
      const xt6TextY = this.printPositionConfig.xt6TextY;
      commands.push(`TEXT ${xt6TextX},${xt6TextY},"${this.printPositionConfig.xt6FontSize}",0,1,1,"XT6"`);
      ...
      // DATE - Ajout de la date formatée
      if (ticket.formattedDate) {
        const dateX = this.printPositionConfig.dateX;
        const dateY = this.printPositionConfig.dateY;
        commands.push(`TEXT ${dateX},${dateY},"${this.printPositionConfig.dateFontSize}",0,1,1,"${ticket.formattedDate}"`);
      }
  ...
  private generatePrintableHTML(): string {
      ...
      ticketsWithDate.forEach((ticket, index) => {
    ...
    return `
      <div class="ticket-label" data-ticket="${ticketNumber}">
        ...
        <div class="info-section">
          ...
          <div class="ticket-date">${ticket.formattedDate || ''}</div>
        </div>
      </div>
    `;
  }
    ...
    printTickets(request: PrintTicketRequest): Observable<PrintTicketResponse> {
      ...
      if (request.printDate) {
        const formattedDate = this.formatDateShort(new Date(request.printDate));
        request = { ...request, formattedDate };
      } else {
        const formattedDate = this.formatDateShort(new Date());
        request = { ...request, formattedDate };
      }
