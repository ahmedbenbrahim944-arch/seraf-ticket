...
export class PrintService {
  ...
  // Configuration d'impression pour XT4
  ...
  // POSITIONS EN PIXELS pour XT4 - MODIFIÉES
  ...
  /**
   * IMPRESSION TSPL POUR FORMAT LIGNE CONTINUE XT4
   */
  printWithLineTSPL(tickets: any[], copies: number = 1): Promise<{ success: boolean; message: string }> {
    return new Promise((resolve, reject) => {
      try {
        ...
        console.log(`Génération TSPL ligne XT4 pour ${tickets.length} tickets`);
  ...
  /**
   * GÉNÉRATION TSPL POUR FORMAT LIGNE CONTINUE XT4
   */
  ...
  private generatePrintableHTML(): string {
    ...
    return `
      ...
      <html>
        <head>
          <title>Étiquettes XT4 - Format Ligne 104x15mm</title>
          <style>
            @page { 
              size: ${lineWidth}mm ${lineHeight}mm;
              margin: 0mm; 
